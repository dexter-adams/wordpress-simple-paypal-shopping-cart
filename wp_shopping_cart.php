...
if( !defined( 'WP_CART_MANAGEMENT_PERMISSION' ) )
{//This will allow the user to define custom capability for this constant in wp-config file
...
function wpspsc_process_pp_smart_checkout()
{
	...
	if( empty( $data ) )
	{
		wp_send_json( [ 'success' => false, 'errMsg' => __( 'Empty payment data received.', "wordpress-simple-paypal-shopping-cart" ) ] );
	}
	...
	if( $ipn_data_success !== true )
	{
		...
		wp_send_json( [ 'success' => false, 'errMsg' => $ipn_data_success ] );
	}
	...
	if( $res !== true )
	{
		wp_send_json( [ 'success' => false, 'errMsg' => $res ] );
	}
	...
	{
		wp_send_json( [ 'success' => false, 'errMsg' => __( 'Error occured during payment processing. Check debug log for additional details.', "wordpress-simple-paypal-shopping-cart" ) ] );
	}
}
...
function wpspc_cart_actions_handler()
{
	...
	{//Add to cart action
		...
		if( isset( $_POST['price'] ) )
		{
			...
			{//Security check failed. Price field has been tampered. Fail validation.
		...
		if( isset( $_POST['shipping'] ) )
		{
			...
			{//Shipping price validation failed
		...
		if( $count == 1 )
		{
			...
			$product = [
				'name' => $post_wspsc_product,
				'price' => $price,
				'price_orig' => $price,
				'quantity' => $count,
				'shipping' => $shipping,
				'cartLink' => $post_cart_link,
				'item_number' => $post_item_number
			];
...
function print_wp_cart_button_new( $content )
{
	...
	foreach( $matches[0] as $match )
	{
		...
		if( sizeof( $pieces ) > 2 )
		{
			...
			{//Shipping parameter has non-numeric value. Discard it and set it to 0.
				$pieces['2'] = 0;
			}
...
function print_wp_cart_button_for_product( $name, $price, $shipping = 0, $var1 = '', $var2 = '', $var3 = '', $atts = [] )
{
	$addcart = get_option( 'addToCartButtonName' );
	if( !$addcart || ( $addcart == '' ) )
	{
...
class WP_PayPal_Cart_Widget extends WP_Widget
{
	...
	{
		parent::__construct( 'wp_paypal_shopping_cart_widgets', 'WP Paypal Shopping Cart', [ 'description' => 'WP Paypal Shopping Cart Widget' ] );
	}
...
{
	if( isset( $_GET['page'] ) && $_GET['page'] == 'wspsc-discounts' )
	{ //simple paypal shopping cart discount page
		wp_enqueue_style( 'jquery-ui-style', '//code.jquery.com/ui/1.11.1/themes/smoothness/jquery-ui.css' );
		wp_register_script( 'wpspsc-admin', WP_CART_URL . '/lib/wpspsc_admin_side.js', [ 'jquery', 'jquery-ui-datepicker' ] );
...
{
	wp_enqueue_style( 'wspsc-admin-style', WP_CART_URL . '/assets/wspsc-admin-styles.css', [], WP_CART_VERSION );
}

function wspsc_front_side_enqueue_scripts()
{
	wp_enqueue_style( 'wspsc-style', WP_CART_URL . '/wp_shopping_cart_style.css', [], WP_CART_VERSION );
}
